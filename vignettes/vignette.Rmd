...
**Note.** `pio` relies on [crayon](https://github.com/r-lib/crayon), which provides core functions to add colours to terminals. This vignette is generated using the [fansi](https://github.com/brodieG/fansi) package that renders ANSI modified strings from `crayon` into HTML code.

```{r, comment="", results="asis", echo=FALSE, message=FALSE}
